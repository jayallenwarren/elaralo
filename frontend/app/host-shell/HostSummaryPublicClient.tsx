...
export default function HostSummaryPublicClient() {
  ...
  const chips = [
    safeText(publicProfile.companion_type || (isAiCompanionCard ? "AI Companion" : "")),
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 220px) minmax(0, 1fr)", gap: 20, alignItems: "start" }}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              <h1 style={{ margin: 0, fontSize: 42, lineHeight: 1.1 }}>
                {safeText(publicProfile.public_display_name || publicProfile.stage_name || "Companion profile")}
              </h1>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {chips.map((chip) => (
                  <span key={chip} style={{ padding: "6px 10px", borderRadius: 999, background: "rgba(17,24,39,0.06)", fontSize: 13, color: "#374151" }}>
                    {chip}
                  </span>
                ))}
                {approvedLabel ? <span style={{ padding: "6px 10px", borderRadius: 999, background: "rgba(17,24,39,0.06)", fontSize: 13, color: "#374151" }}>Approved {approvedLabel}</span> : null}
              </div>
