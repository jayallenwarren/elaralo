...
const STREAM_BROADCAST_URL_TEMPLATE = process.env.NEXT_PUBLIC_STREAM_BROADCAST_URL_TEMPLATE || "";
...
const [showBroadcaster, setShowBroadcaster] = useState<boolean>(true);

const streamBroadcastUrl = useMemo(() => {
  const ref = String(streamEventRef || "").trim();
  if (!ref) return "";

  // Prefer an explicit template, since providers vary on the broadcaster URL shape.
  // Supported placeholders: {event_ref} or {EVENT_REF}
  const tpl = String(STREAM_BROADCAST_URL_TEMPLATE || "").trim();
  if (tpl) {
    const enc = encodeURIComponent(ref);
    return tpl.split("{event_ref}").join(enc).split("{EVENT_REF}").join(enc);
  }

  const base = String(STREAM_URL || "").trim().replace(/\/+$/, "");
  if (!base) return "";

  // Default guess for BeeStreamed broadcaster UI.
  return `${base}/broadcast?id=${encodeURIComponent(ref)}`;
}, [streamEventRef]);
...
{streamCanStart && liveProvider === "stream" && (
  <button
    type="button"
    onClick={() => setShowBroadcaster((v) => !v)}
    title={showBroadcaster ? "Hide broadcaster UI" : "Show broadcaster UI"}
    style={{
      padding: "10px 14px",
      borderRadius: 10,
      border: "1px solid #111",
      background: showBroadcaster ? "#111" : "#fff",
      color: showBroadcaster ? "#fff" : "#111",
      cursor: "pointer",
      fontWeight: 600,
    }}
  >
