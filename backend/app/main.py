...
@app.get("/brand/{brand}/companion/{companion_type}/{filename:path}", name="brand_companion_asset")
async def brand_companion_asset(request: Request, brand: str, companion_type: str, filename: str):
    """Serve brand companion images from the App Service Linux filesystem.

    Elaralo AI companion images are sourced from:
      /home/site/brand/elaralo/companion/ai/

    This route intentionally serves files only; it does not expose directory listings.
    """
    _ = request
    path, resolved_name = _brand_companion_asset_path(brand, companion_type, filename)
    if not path or not os.path.isfile(path):
        raise HTTPException(status_code=404, detail="companion asset not found")
    media_type = mimetypes.guess_type(path)[0] or "application/octet-stream"
    headers = {
        "Content-Disposition": f'inline; filename="{resolved_name}"',
...
@app.get("/companion/headshot/{filename:path}", name="elaralo_ai_legacy_headshot_asset")
async def elaralo_ai_legacy_headshot_asset(request: Request, filename: str):
    """Compatibility alias for Elaralo AI headshots."""
    _ = request
    path, resolved_name = _brand_companion_asset_path("Elaralo", "ai", filename)
    if not path or not os.path.isfile(path):
        raise HTTPException(status_code=404, detail="companion headshot not found")
    media_type = mimetypes.guess_type(path)[0] or "application/octet-stream"
    headers = {
        "Content-Disposition": f'inline; filename="{resolved_name}"',
        "Cache-Control": "public, max-age=3600",
...
def _parse_ai_companion_card_meta(raw: Any) -> Dict[str, str]:
    """Parse an AI companion filename/key into Companion Card metadata.

    For the Elaralo brand, the canonical AI companion_key is the exact
    extensionless filename stem stored in /home/site/brand/elaralo/companion/ai/.
    Example: Adriana-Female-Hispanic-Millennials.jpeg ->
    Adriana-Female-Hispanic-Millennials.
    """
...
# -----------------------------------------------------------------------------
# Elaralo AI Companion Linux image catalog
# -----------------------------------------------------------------------------
# Source of truth for Elaralo AI companions:
#   /home/site/brand/elaralo/companion/ai/
# The filename stem is the companion_key and the metadata source.
_ELARALO_AI_COMPANION_DIR_DEFAULT = "/home/site/brand/elaralo/companion/ai"
_ELARALO_HUMAN_COMPANION_DIR_DEFAULT = "/home/site/brand/elaralo/companion/human"
_ELARALO_COMPANION_IMAGE_EXTENSIONS = (".jpeg", ".jpg", ".png", ".webp")
_ELARALO_AI_COMPANION_CACHE: Dict[str, Any] = {"loaded_at": 0.0, "items": []}
_ELARALO_AI_COMPANION_CACHE_TTL_SECONDS = 30.0
...
def _elaralo_companion_dir_candidates(companion_type: Any) -> List[str]:
    kind = _host_onboarding_safe_str(companion_type).lower()
    ...
    if kind == "ai":
        env_names = (
            "ELARALO_AI_COMPANION_IMAGE_DIR",
            "ELARALO_AI_COMPANION_DIR",
            "AI_COMPANION_IMAGE_DIR",
            "AI_COMPANION_DIR",
        )
        default_dir = _ELARALO_AI_COMPANION_DIR_DEFAULT
    else:
        env_names = (
            "ELARALO_HUMAN_COMPANION_IMAGE_DIR",
            "ELARALO_HUMAN_COMPANION_DIR",
            "HUMAN_COMPANION_IMAGE_DIR",
            "HUMAN_COMPANION_DIR",
        )
        default_dir = _ELARALO_HUMAN_COMPANION_DIR_DEFAULT
    ...
    root = os.getenv("ELARALO_COMPANION_ROOT", "") or os.getenv("BRAND_COMPANION_ROOT", "") or ""
    if root:
        add(os.path.join(root, kind))
        add(os.path.join(root, "elaralo", "companion", kind))

    add(default_dir)
    return out


def _elaralo_ai_companion_stem(raw: Any) -> str:
    stem = _ai_companion_strip_trailing_uuid(_ai_companion_strip_extensions(str(raw or "")))
    stem = re.sub(r"\s+", "-", stem).strip("- ")
...
def _brand_companion_asset_path(brand: Any, companion_type: Any, filename: Any) -> Tuple[str, str]:
    """Resolve a brand companion image path without allowing traversal."""
    if not _is_elaralo_core_brand(brand):
        return "", ""
    kind = _host_onboarding_safe_str(companion_type).lower()
    if kind in ("ai_companion", "artificial_intelligence"):
        kind = "ai"
    if kind in ("human_companion", "host"):
        kind = "human"
    if kind not in ("ai", "human"):
        return "", ""

    raw_filename = unquote(_host_onboarding_safe_str(filename))
    if not raw_filename:
        return "", ""
    if raw_filename != os.path.basename(raw_filename):
        return "", ""
    if any(x in raw_filename for x in ("..", "/", "\\")):
        return "", ""

    wanted = raw_filename.lower()
    wanted_stem = _ai_companion_strip_extensions(raw_filename).lower()
    for folder in _elaralo_companion_dir_candidates(kind):
        ...
        for entry in entries:
            ...
            if not os.path.isfile(full):
                continue
            low = entry.lower()
            if not low.endswith(_ELARALO_COMPANION_IMAGE_EXTENSIONS):
                continue
            if low == wanted or _ai_companion_strip_extensions(entry).lower() == wanted_stem:
                return full, entry
    return "", ""


def _elaralo_ai_companion_assets(force_refresh: bool = False) -> List[Dict[str, str]]:
    now = time.time()
    if not force_refresh:
        loaded_at = float(_ELARALO_AI_COMPANION_CACHE.get("loaded_at") or 0.0)
        cached = _ELARALO_AI_COMPANION_CACHE.get("items") or []
        if loaded_at and now - loaded_at < _ELARALO_AI_COMPANION_CACHE_TTL_SECONDS:
            return [dict(item) for item in cached if isinstance(item, dict)]

    items: List[Dict[str, str]] = []
    seen: Set[str] = set()
    for folder in _elaralo_companion_dir_candidates("ai"):
        if not os.path.isdir(folder):
            continue
        try:
        ...
        for entry in entries:
            ...
            if not low.endswith(_ELARALO_COMPANION_IMAGE_EXTENSIONS):
                continue
            if any(token in low for token in ("logo", "placeholder", "default")):
                continue
            full = os.path.join(folder, entry)
            if not os.path.isfile(full):
                continue
            stem = _elaralo_ai_companion_stem(entry)
            if not stem:
                continue
            key = stem.lower()
            if key in seen:
    ...
    _ELARALO_AI_COMPANION_CACHE["loaded_at"] = now
    _ELARALO_AI_COMPANION_CACHE["items"] = [dict(item) for item in items]
    return items
...
def _elaralo_ai_first_token(key: Any) -> str:
    ...
    stem = re.sub(r"\s+", "-", stem).strip("- ")
    return stem.split("-", 1)[0].strip() if stem else ""


def _resolve_elaralo_ai_companion_key(brand: Any, raw_avatar: Any) -> str:
    """Resolve exact Elaralo AI companion_key from the Linux image filename catalog."""
    cleaned = _ai_companion_strip_trailing_uuid(_ai_companion_strip_extensions(str(raw_avatar or "")))
    cleaned = re.sub(r"\s+", "-", cleaned).strip("- ")
    if not cleaned or not _is_elaralo_core_brand(brand):
        return cleaned

    assets = _elaralo_ai_companion_assets()
    for item in assets:
        stem = _host_onboarding_safe_str(item.get("stem"))
        file_name = _host_onboarding_safe_str(item.get("file_name"))
        if cleaned.lower() in (stem.lower(), file_name.lower(), _ai_companion_strip_extensions(file_name).lower()):
            return stem

    if "-" not in cleaned:
        matches = [item for item in assets if _elaralo_ai_first_token(item.get("stem")).lower() == cleaned.lower()]
        if len(matches) == 1:
            return _host_onboarding_safe_str(matches[0].get("stem"))

    return cleaned


def _elaralo_ai_companion_asset_for_key(brand: Any, raw_avatar: Any, public_base_url: str = "") -> Dict[str, str]:
    if not _is_elaralo_core_brand(brand):
        return {}
    resolved_key = _resolve_elaralo_ai_companion_key(brand, raw_avatar)
    if not resolved_key:
        return {}
    target = resolved_key.lower()
    target_file = os.path.basename(str(raw_avatar or "")).lower()
    for item in _elaralo_ai_companion_assets():
        stem = _host_onboarding_safe_str(item.get("stem"))
        file_name = _host_onboarding_safe_str(item.get("file_name"))
        if target in (stem.lower(), _ai_companion_strip_extensions(file_name).lower()) or target_file == file_name.lower():
            out = dict(item)
            out["url"] = _elaralo_ai_companion_file_url(file_name, public_base_url=public_base_url)
            return out
    return {}



def _companion_public_api_base_url(request: Optional[Request] = None, fallback_base: str = "") -> str:
    """Return the public API origin used to serve Linux companion images.

    Companion image files live on the Linux App Service filesystem, so browser
    URLs for these images must point at the API host, not the Static Web App
    host.  Prefer the current request host because the catalog/summary request
    is already being made to the public API origin.  Use API-oriented env vars as
    fallbacks; intentionally do not prefer APP_BASE_URL because that is commonly
    the front-end/static-app origin.
    """
    ...
    env_candidates = [
        os.getenv("ELARALO_PUBLIC_API_BASE_URL", ""),
    ...
    for cand in env_candidates:
        try:
            ...
            norm = str(cand or "").strip().rstrip("/")
        if norm:
            return norm
    return ""


def _elaralo_ai_companion_file_url(file_name_or_stem: Any, public_base_url: str = "") -> str:
    name = os.path.basename(_host_onboarding_safe_str(file_name_or_stem))
    if not name:
        return ""
    if not name.lower().endswith(_ELARALO_COMPANION_IMAGE_EXTENSIONS):
        path, resolved_name = _brand_companion_asset_path("Elaralo", "ai", name)
        _ = path
        name = resolved_name or name
    encoded_name = quote(name, safe="")
    path = f"/brand/elaralo/companion/ai/{encoded_name}"
    base = _host_onboarding_safe_str(public_base_url)
    if not base:
        try:
            base = _companion_public_api_base_url(fallback_base=public_base_url)
        except Exception:
            base = _host_onboarding_safe_str(public_base_url)
    if base:
        try:
...
def _ai_headshot_url_from_mapping_or_file(brand: Any, avatar_key: Any, mapping: Dict[str, Any], public_base_url: str = "") -> str:
    if _is_elaralo_core_brand(brand):
        asset = _elaralo_ai_companion_asset_for_key(brand, avatar_key, public_base_url=public_base_url)
        static_url = _host_onboarding_safe_str(asset.get("url"))
        if static_url:
            return static_url
    return _host_onboarding_safe_str((mapping or {}).get("headshot_url") or (mapping or {}).get("headshotUrl"))


def _ai_companion_public_payload(brand: str, raw_avatar: str, public_base_url: str = "") -> Dict[str, Any]:
    brand_name = _host_onboarding_safe_str(brand) or "Elaralo"
    ...
    meta = _parse_ai_companion_card_meta(resolved_avatar)
    ...
    mapping = _lookup_companion_mapping_for_ai_media(brand_name, avatar_key) if _is_elaralo_core_brand(brand_name) else (_lookup_companion_mapping(brand_name, avatar_key) or {})
    static_asset = _elaralo_ai_companion_asset_for_key(brand_name, avatar_key, public_base_url=public_base_url)
    if _is_elaralo_core_brand(brand_name) and not static_asset and not _ai_companion_key_has_metadata(avatar_key):
        raise HTTPException(
            status_code=404,
            detail={
                "error": "Elaralo AI companion image filename not found",
                "avatar": raw_avatar,
                "expected_source_dir": _ELARALO_AI_COMPANION_DIR_DEFAULT,
                "rule": "AI companion_key must match the extensionless hyphenated image filename stem.",
            },
        )
    ...
    headshot_url = _ai_headshot_url_from_mapping_or_file(brand_name, avatar_key, mapping, public_base_url=public_base_url)
    ...
    headshot_asset = {
        ...
        "url": headshot_url,
        "content_type": mimetypes.guess_type(headshot_file_name)[0] or ("image/*" if headshot_url else ""),
    ...
    public_profile = {
        ...
        "avatar": avatar_key,
        "companion_key": avatar_key,
        "mapping_avatar": _host_onboarding_safe_str((mapping or {}).get("mapping_avatar")),
...
_PLATFORM_INLINE_FILENAME_RE = re.compile(
...
def _normalize_platform_filename_candidate(value: str) -> str:
    candidate = str(value or "").strip().strip("\"'")
    if not candidate:
        return ""
    candidate = re.split(r'\s+https?://', candidate, maxsplit=1)[0].strip()
...
def _platform_content_filename_from_text(text: str) -> str:
    ...
    placeholder_match = _PLATFORM_PLACEHOLDER_WITH_FILENAME_RE.match(raw.strip())
    if placeholder_match:
        candidate = _normalize_platform_filename_candidate(placeholder_match.group(1) or "")
        if candidate:
            return candidate

    for match in _PLATFORM_FILENAME_IN_TEXT_RE.finditer(raw):
        candidate = _normalize_platform_filename_candidate(match.group(1) or "")
        if candidate:
            return candidate

    inline_match = _PLATFORM_INLINE_FILENAME_RE.search(raw)
    if inline_match:
        candidate = _normalize_platform_filename_candidate(inline_match.group(1) or "")
        if candidate:
            return candidate
...
def _platform_content_message_is_generic(text: str) -> bool:
    return bool(_PLATFORM_GENERIC_PLACEHOLDER_RE.match(str(text or "").strip()))


def _extract_platform_content_filenames(messages: List[Dict[str, str]]) -> List[str]:
    out: List[str] = []
    seen: Set[str] = set()
    for item in messages or []:
        if str(item.get("role") or "").strip().lower() != "assistant":
            continue
        name = _platform_content_filename_from_text(str(item.get("content") or ""))
        if not name:
            continue
        key = name.lower()
        if key in seen:
...
def _repair_generated_summary_for_platform_content(summary: str, platform_filenames: List[str]) -> str:
    text = str(summary or "").strip()
    if not text:
        return text
    if not _platform_content_message_is_generic(text):
        return text
    if not platform_filenames:
        return text
    unique_files = [str(x or "").strip() for x in platform_filenames if str(x or "").strip()]
    if not unique_files:
        return text
    if len(unique_files) == 1:
        return _platform_content_placeholder(unique_files[0])
...
def _sanitize_message_content_for_llm(role: str, content: str) -> str:
    ...
    if role_norm == "assistant":
        if _platform_content_message_is_generic(raw):
            return ""
        placeholder_name = _platform_content_filename_from_text(raw)
        if placeholder_name and _PLATFORM_PLACEHOLDER_WITH_FILENAME_RE.match(raw):
            return _platform_content_placeholder(placeholder_name)

    cleaned, removed = _sanitize_platform_content_text(content)
    if removed and role_norm == "assistant":
...
def _sanitize_summary_for_platform_content(text: str) -> str:
    ...
    if _platform_content_message_is_generic(raw):
        return ""
    file_name = _platform_content_filename_from_text(raw)
    if file_name and _PLATFORM_PLACEHOLDER_WITH_FILENAME_RE.match(raw):
        return _platform_content_placeholder(file_name)
    cleaned, removed = _sanitize_platform_content_text(raw)
...
_CHAT_SUMMARY_HISTORY_CLEANUP_INTERVAL_S = float(os.getenv("CHAT_SUMMARY_HISTORY_CLEANUP_INTERVAL_S", "86400") or "86400")
...
def _summary_history_dedupe_expr(alias: str = "") -> str:
    ...
    return (
        f"CASE "
        f"WHEN COALESCE({prefix}session_id, '') <> '' THEN 'session:' || COALESCE({prefix}session_id, '') "
        f"WHEN COALESCE({prefix}conversation_hash, '') <> '' THEN 'hash:' || COALESCE({prefix}conversation_hash, '') "
        f"ELSE 'legacy:' || COALESCE({prefix}summary, '') END"
    )
    ...
    conn.execute(
        ...
        WHERE id IN (
          ...
          JOIN (
            ...
            GROUP BY brand, avatar, member_id, session_id
            HAVING COUNT(*) > 1
          ) d
            ON h.brand = d.brand
           AND h.avatar = d.avatar
           AND h.member_id = d.member_id
           AND h.session_id = d.session_id
           AND h.id <> d.keep_id
        );
        """
    )
    ...
    conn.execute(
        ...
        WHERE id IN (
          ...
          JOIN (
            ...
            FROM chat_summary_history
            WHERE COALESCE(session_id, '') = ''
              AND COALESCE(conversation_hash, '') <> ''
        ...
        try:
            ...
            if session_id_s:
                row = conn.execute(
                    ...
                    LIMIT 1;
                    """,
                    (brand_s, avatar_s, member_id_s, session_id_s),
                ).fetchone()
                if row is not None:
                    keep_id = int(row[0] or 0)
                    conn.execute(
                        """
                        UPDATE chat_summary_history
                        SET create_datetime = datetime('now'),
                            user_name = ?,
                            reason = ?,
                            conversation_hash = ?,
                            summary = ?
                        WHERE id = ?;
                        """,
                        (user_name_s, reason_s, (conversation_hash_s or None), summary_s, keep_id),
                    )
                    conn.execute(
                        """
                        DELETE FROM chat_summary_history
                        WHERE brand = ?
                          ...
                          AND id <> ?;
                        """,
                        (brand_s, avatar_s, member_id_s, session_id_s, keep_id),
                    )
                    conn.commit()
                    return
...
async def chat(request: Request):
    ...
    if not usage_ok and not is_minutes_balance_query:
        ...
        # Even if the member is paywalled, scheduled/retroactive content delivery must still occur.
...
def _host_active_session_group_key(item: Dict[str, Any]) -> str:
    if not isinstance(item, dict):
        return ""
    brand = str(item.get("brand") or "").strip().lower()
    avatar = str(item.get("avatar") or "").strip().lower()
    member_id = str(item.get("member_id") or "").strip().lower()
    user_name = str(item.get("user_name") or "").strip().lower()
    summary = re.sub(r"\s+", " ", str(item.get("summary") or "").strip().lower())[:240]
    if member_id:
        return f"member:{brand}:{avatar}:{member_id}:{summary or '-'}"
    if user_name:
        return f"user:{brand}:{avatar}:{user_name}:{summary or '-'}"
    return f"session:{str(item.get('session_id') or '').strip()}"


def _dedupe_host_active_session_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    out_by_key: Dict[str, Dict[str, Any]] = {}
    ordered: List[Dict[str, Any]] = sorted(
        [dict(r) for r in (rows or []) if isinstance(r, dict)],
        key=lambda r: float(r.get("last_seen") or 0.0),
        reverse=True,
    )
    for row in ordered:
        key = _host_active_session_group_key(row) or f"session:{str(row.get('session_id') or '').strip()}"
        prev = out_by_key.get(key)
        if prev is None:
            out_by_key[key] = row
            continue
        keep = prev
        keep["unread"] = max(int(keep.get("unread") or 0), int(row.get("unread") or 0))
        if not str(keep.get("summary") or "").strip() and str(row.get("summary") or "").strip():
            keep["summary"] = row.get("summary")
            keep["summary_source"] = row.get("summary_source")
        if not str(keep.get("user_name") or "").strip() and str(row.get("user_name") or "").strip():
            keep["user_name"] = row.get("user_name")
        if not str(keep.get("member_id") or "").strip() and str(row.get("member_id") or "").strip():
            keep["member_id"] = row.get("member_id")
        keep["override_active"] = bool(keep.get("override_active") is True or row.get("override_active") is True)
...
async def host_ai_chats_active(req: HostAiChatsActiveRequest):
    ...
    sessions_out = _dedupe_host_active_session_rows(sessions_out)
...
async def host_ai_chats_push_content(req: HostAiChatsPushContentRequest):
    """Host pushes a scheduled content item to the user during override."""
    ...
    # Only the controlling host can push into the session.
    ...
    if not token or not content:
        raise HTTPException(status_code=400, detail="token and content are required")

    trig = int(req.trigger_minute or 0)
    if not trig:
        try:
            trig = int((content or {}).get("triggerMinute") or 0)
        except Exception:
            trig = 0
...
async def host_ai_chats_poll(req: HostAiChatsPollRequest):
    ...
    # Business rule: content is considered delivered once the host has received it.
    try:
        for ev in (out or {}).get("events", []) or []:
            if isinstance(ev, dict) and ev.get("kind") == "content_pending":
                _content_mark_host_received(ev.get("payload") or {})
    except Exception:
        pass
...
def _safe_slug(s: str) -> str:
    ...
    if not s:
        return ""
    # allow a-z, 0-9, underscore, dash. Everything else becomes '-'
...
def _content_brand_slug(session_state: Dict[str, Any]) -> str:
    # Prefer explicit slugs if present.
    for k in (
        "brandKey",
        "brand_key",
        "rebrandingSlug",
...
def _content_parse_filename(name: str) -> Dict[str, str]:
    """Parse branded content filenames.

    Canonical format:
        <SEQ>-<DESCRIPTION>-<TYPE>-<STAGE>.<ext>

    Business rules rely on:
      - DESCRIPTION = 2nd hyphen-delimited field
      - TYPE = 3rd hyphen-delimited field (Photo/Video)

    Some assets may have double extensions (e.g. .png.jpg). We strip everything after the first dot.
    """
    if not name:
        return {}

    # Robustly strip extensions (handles double extensions like .png.jpg)
    stem = name.split(".")[0]
    ...
    type_field3 = parts[2] if len(parts) >= 3 else ""

    # Backwards-compatible inference (more robust if extra hyphens ever appear)
    stage = parts[-1] if len(parts) >= 4 else ""
    type_from_end = parts[-2] if len(parts) >= 4 else type_field3

    # Title shown to host / metadata; preserve old behavior (join middle parts)
    title = "-".join(parts[1:-2]).replace("_", " ").strip() if len(parts) >= 4 else description

    kind = "unknown"
    ct = (type_from_end or type_field3 or "").lower()
    if ct in ("photo", "image", "pic", "jpeg", "jpg", "png"):
        kind = "image"
    elif ct in ("video", "mp4", "mov", "webm", "m4v"):
        kind = "video"

    return {
        "sequence": seq,
        "description": (description or title or stem).strip(),
        "title": (title or description or stem).strip(),
        "kind": kind,
        "stage": stage.strip(),
        # Human-facing type string (prefer 3rd hyphen field)
...
def _content_pick_next_file(brand_slug: str, folder: str, last_sequence: Optional[str]) -> Optional[str]:
    files = _content_list_files(brand_slug, folder)
    if not files:
        return None
    ...
    if last_sequence:
        try:
            last_n = int(str(last_sequence).strip())
        except Exception:
            last_n = -1

    # Pick the first file whose numeric prefix is greater than last_sequence.
    for fn in files:
        try:
            n = int(fn.split("-", 1)[0])
        except Exception:
    ...
    # Wrap-around fallback: start from the beginning.
    return files[0]


def _content_state_get(conn: sqlite3.Connection, member_id: str, folder: str) -> Dict[str, Any]:
    row = conn.execute(
        "SELECT * FROM user_content_state WHERE member_id=? AND content_folder=?",
        (member_id, folder),
    ).fetchone()
    return dict(row) if row else {}


def _content_state_upsert(conn: sqlite3.Connection, member_id: str, folder: str, updates: Dict[str, Any]) -> None:
    # SQLite UPSERT
    cols = ["member_id", "content_folder"] + list(updates.keys())
    vals = [member_id, folder] + list(updates.values())
    placeholders = ",".join(["?"] * len(cols))
    assignments = ",".join([f"{c}=excluded.{c}" for c in updates.keys()])
    conn.execute(
        f"INSERT INTO user_content_state ({','.join(cols)}) VALUES ({placeholders}) "
        f"ON CONFLICT(member_id, content_folder) DO UPDATE SET {assignments}",
        vals,
    )


def _content_roll_window_if_needed(
    conn: sqlite3.Connection,
    *,
    member_id: str,
    folder: str,
    ...
    base_used = float(state.get("window_base_used_seconds") or 0)

    expired = False
    if window_start is not None:
        if now_epoch - window_start >= _CONTENT_WINDOW_SECONDS:
            expired = True
    if window_cycle_id and window_cycle_id != str(cycle_id or ""):
        expired = True
    ...
    if not state.get("window_cycle_id"):
        state["window_cycle_id"] = str(cycle_id or "")
    if state.get("window_base_used_seconds") is None:
...
def _content_next_trigger_minute(state: Dict[str, Any]) -> Optional[int]:
    ...
    sent = int(state.get("window_sent_count") or 0)
    last_trig = state.get("last_trigger_minute")

    if sent <= 0 or last_trig is None:
        return _CONTENT_TRIGGER_START_MINUTE
    try:
        return int(last_trig) + _CONTENT_TRIGGER_STEP_MINUTES
    except Exception:
        return _CONTENT_TRIGGER_START_MINUTE
...
def _ai_override_has_token_event(session_id: str, token: str, kind: str) -> bool:
    if not token:
        return False
    with _AI_OVERRIDE_LOCK:
...
def _content_mark_host_received(payload: Dict[str, Any]) -> None:
    ...
    member_key = f"{brand_slug}::{member_id}" if brand_slug else member_id
    folder = str(payload.get("content_folder") or "").strip() or "friend"
    trig = payload.get("trigger_minute")
    try:
        trig_i = int(trig)
    except Exception:
        trig_i = None
    ...
    try:
        ...
        if row and str(row["pending_token"] or "") == token:
            sent_count = int(row["window_sent_count"] or 0) + 1
            complete = 0
            if folder == "friend":
                complete = 1
            elif str(stage or "").strip().lower() == "end":
                complete = 1
...
def _my_elaralo_card_matches_filters(
    card: Dict[str, Any],
    companion_type: str = "",
    generation: str = "",
    ethnicity: str = "",
    ...
    if not _my_elaralo_catalog_filter_match(card.get("companion_type"), companion_type):
        return False
    if not _my_elaralo_catalog_filter_match(card.get("generation"), generation):
        return False
    if not _my_elaralo_catalog_filter_match(card.get("ethnicity"), ethnicity):
...
def _my_elaralo_ai_file_cards(brand_name: str, public_base_url: str = "") -> List[Dict[str, Any]]:
    cards: List[Dict[str, Any]] = []
    for asset in _elaralo_ai_companion_assets():
        companion_key = _host_onboarding_safe_str(asset.get("stem"))
        file_name = _host_onboarding_safe_str(asset.get("file_name"))
        if not companion_key or not file_name:
            continue
        payload = _ai_companion_public_payload(brand_name, companion_key, public_base_url=public_base_url)
        public_profile = dict(payload.get("public_profile") or {})
        public_page = dict(payload.get("public_page") or {})
        ...
        cards.append({
            ...
            "headshot_url": _host_onboarding_safe_str(((public_page.get("headshot_asset") or {}).get("url")) or _elaralo_ai_companion_file_url(file_name, public_base_url=public_base_url)),
            "headshot_file_name": file_name,
            "approved_version_id": "",
...
def _my_elaralo_companion_cards_sync(
    ...
    def add_card(card: Dict[str, Any]) -> None:
        ...
        if not _my_elaralo_card_matches_filters(card, companion_type, generation, ethnicity, gender):
            return
        ctype = _host_onboarding_safe_str(card.get("companion_type") or card.get("companionType") or "AI")
    ...
    ai_cards = _my_elaralo_ai_file_cards(brand_name, public_base_url=public_base_url)
    if want_type != "human":
...
async def host_onboarding_public_summary(
    ...
    if not resolved_version_id and not resolved_member_id and resolved_avatar:
        payload = _ai_companion_public_payload(resolved_brand, resolved_avatar, public_base_url=_companion_public_api_base_url(request=request))
        return {
            "ok": True,
            "version": None,
            **payload,
