...
async def wix_payment_link_payment_updated(request: Request):
    ...
    try:
        if pub_key:
            payload = jwt.decode(token, pub_key, algorithms=["RS256"], options={"verify_aud": False})  # type: ignore
        else:
            # Allow dev environments without verification (NOT recommended for production).
