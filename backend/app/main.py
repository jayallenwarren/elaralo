...
def _load_companion_mappings_sync() -> None:
    ...
    for r in rows:
        ...
        d[key] = {
            ...
            # Phonetic pronunciation for companion name (TTS only; display name remains unchanged).
            "phonetic": str(get_col("phonetic", "Phonetic", "PHONETIC", default="") or ""),
...
def _is_minutes_balance_question(text: str) -> bool:
    ...
    # Common phrasing without an explicit left/remaining token.
    # Examples:
    #   - "How many minutes do I have?"
    #   - "How much time do I have?"
    if re.search(r"\bhow\s+many\b.*\bminutes?\b", t):
        return True
    if re.search(r"\bhow\s+much\b.*\btime\b", t):
        return True
    if re.search(r"\bminutes?\b.*\bdo\s+i\s+have\b", t):
        return True
    if re.search(r"\btime\b.*\bdo\s+i\s+have\b", t):
        return True
...
# ----------------------------
# TTS Text Normalization (phonetic + branding + link stripping)
# ----------------------------
_CAMEL_SPLIT_RE = re.compile(r"(?<=[a-z])(?=[A-Z])")


def _split_camel_case(s: str) -> str:
    """Turns 'DulceMoon' -> 'Dulce Moon' (best-effort)."""
    s = (s or "").strip()
    if not s:
        return ""
...
def _strip_links_for_tts(s: str) -> str:
    """Remove/neutralize URLs so TTS doesn't read them aloud."""
    s = s or ""
...
def _find_mapping_by_voice_id(voice_id: str) -> Optional[Dict[str, Any]]:
    vid = (voice_id or "").strip()
    if not vid:
        return None
    try:
        # Small table; linear scan is fine.
        for _k, m in _COMPANION_MAPPINGS.items():
            if str(m.get("eleven_voice_id") or "").strip() == vid:
                return m
    except Exception:
        return None
    return None
...
def _preprocess_tts_text(text: str, voice_id: str) -> str:
    """
    Always runs before audio generation.
    - Strips URLs/markdown link targets (paywall messages often include links)
    - Expands CamelCase branding tokens for better pronunciation
    - Applies companion phonetic pronunciation when available
    """
    t = (text or "").strip()
    if not t:
        return t

    m = _find_mapping_by_voice_id(voice_id)
    brand = str(m.get("brand") or "") if isinstance(m, dict) else ""
    avatar_name = str(m.get("avatar") or "") if isinstance(m, dict) else ""
    phonetic = str(m.get("phonetic") or "") if isinstance(m, dict) else ""

    # 1) Strip URLs so we don't read domains like "dulcemoon.net" aloud.
    t = _strip_links_for_tts(t)

    # 2) Expand CamelCase brand tokens (DulceMoon -> Dulce Moon)
    if brand:
        brand_no_space = re.sub(r"\s+", "", brand)
        brand_spoken = _split_camel_case(brand_no_space)

        # Replace brand token occurrences (case-insensitive)
        if brand_spoken:
            t = re.sub(re.escape(brand_no_space), brand_spoken, t, flags=re.IGNORECASE)

        # Also handle bare domains without scheme (dulcemoon.net)
        if brand_spoken:
            # brand_no_space might be CamelCase; domains are often lowercase.
            t = re.sub(
                re.escape(brand_no_space.lower()) + r"\.(net|com|org|io|co)\b",
                lambda m2: f"{brand_spoken} dot {m2.group(1)}",
                t,
                flags=re.IGNORECASE,
            )

    # 3) Apply companion phonetic pronunciation (TTS only).
    if avatar_name and phonetic:
        # Word-boundary replacement, case-insensitive.
        t = re.sub(rf"\b{re.escape(avatar_name)}\b", phonetic, t, flags=re.IGNORECASE)

    # 4) Collapse whitespace after removals.
...
def _tts_audio_url_sync(session_id: str, voice_id: str, text: str) -> str:
    ...
    # Pre-TTS normalization (phonetic + branding + link stripping) is applied here so
    # display text remains unchanged but spoken audio is consistent.
    tts_text = _normalize_tts_text_by_voice_id(voice_id=voice_id, text=text)
    if not tts_text:
        tts_text = text
